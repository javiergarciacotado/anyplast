		...
		<c:choose>
			<c:when test="<%= editable && ExpandoColumnPermissionUtil.contains(permissionChecker, company.getCompanyId(), className, ExpandoTableConstants.DEFAULT_TABLE_NAME, name, ActionKeys.UPDATE) %>">
				<% 
					...
					if (name.equals("companyWeb") || name.equals("companyCountrySelect") || name.equals("companyProvinceSelect"))
							isRequired = false;
				%>
				
				<aui:field-wrapper label="<%= label ? localizedName : StringPool.BLANK %>" required="<%= isRequired %>">
						...
						<c:otherwise>
							...
							<c:choose>
								...
								<c:otherwise>
									...
									<input class="lfr-input-text <%= !name.equals("companyWeb") ? "aui-field-required":StringPool.BLANK %>" id="<%= randomNamespace %><%= HtmlUtil.getAUICompatibleId(name) %>" name="<portlet:namespace />ExpandoAttribute--<%= HtmlUtil.escapeAttribute(name) %>--"
										style="
											<c:if test="<%= propertyWidth > 0 %>">
												width: <%= propertyWidth %>px;
											</c:if>"
